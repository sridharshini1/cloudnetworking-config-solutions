...
var (
	...
	firewallRuleName       = "allow-ssh-custom-ranges-workbench"
)

func TestWorkbenchFirewallRuleProperties(t *testing.T) {
	var (
		tfVars = map[string]any{
			...
			"ingress_rules": map[string]any{
				"allow-ssh-custom-ranges-workbench": map[string]any{
					...
					"source_ranges": []string{"192.168.0.0/16"},
					"target_tags":   []string{"allow-ssh-custom-ranges-workbench"},
				},
			},
		}
	)
	...
	firewallRulesOutput := terraform.OutputJson(t, terraformOptions, "workbench_firewall_rules")
	...
	t.Run("Firewall Properties", func(t *testing.T) {
		...
		t.Run("Target Tags", func(t *testing.T) {
			...
			assert.ElementsMatch(t, want, got, "Target tags mismatch")
		})
...
func deleteVPC(t *testing.T, projectID string, network string) {
	...
	if err != nil {
		t.Errorf("Error deleting VPC network %s: %v", network, err)
	}
}
