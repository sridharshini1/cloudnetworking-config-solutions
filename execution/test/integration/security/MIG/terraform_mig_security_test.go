...
func TestMIGFirewallRuleProperties(t *testing.T) {
	...
	t.Run("Firewall Properties", func(t *testing.T) {
		...
		t.Run("Source Ranges", func(t *testing.T) {
			...
			want := []interface{}{"130.211.0.0/22", "35.191.0.0/16"}
		...
		t.Run("Allowed Protocols and Ports", func(t *testing.T) {
			allowRuleData := ruleData.Get("allow").Array()[0].Map() // Get the first "allow" rule

			gotProtocol := allowRuleData["protocol"].String()
			wantProtocol := "tcp"
			assert.Equal(t, wantProtocol, gotProtocol, "Allow rule protocol mismatch")

			gotPorts := allowRuleData["ports"].Array()
			extractedPorts := make([]interface{}, len(gotPorts))
			for i, portResult := range gotPorts {
				extractedPorts[i] = portResult.String()
			}

			wantPorts := []interface{}{"80"} // Updated to match new ingress rules
...
func createVPC(t *testing.T, projectID string, network string) {
	...
	cmd := shell.Command{
		Command: "gcloud",
		Args: []string{text, "networks", "create", network,
			"--project=" + projectID,
			"--format=json",
